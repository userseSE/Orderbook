...
use orderbook::{buy_order, sell_order, initialize_trade, MenuBuyer, MenuSeller};
...
fn main(){
    ...
    buy_order(&mut border, 1, "power".to_string(), 1.0, 5);
    buy_order(&mut border, 2, "power".to_string(), 2.0, 3);
    sell_order(&mut sorder, 1, "power".to_string(), 0.8, 3);
    sell_order(&mut sorder, 2, "power".to_string(), 2.1, 4);
        ...
        for sell_order in &sorder {
            if buy_order.id == sell_order.id {
                if let Some(trade) = initialize_trade(buy_order, sell_order) {
                    ...
                    println!(
                        "Trade initiated: ID:{}, Good:{}, Price: {}, Quantity: {}",
                        trade.id, trade.good, trade.unit_price, trade.quantity
                    );
                } else {
